...
static func draw(b) -> void:
	...
	var y := DeskKit.hero_band(b,
		"%d seat%s open · %d in motion" % [roles.size(), "" if roles.size() == 1 else "s",
			_candidates_in(state, ["applied", "interviewed", "offer"]).size()],
	...
	if not offers.is_empty():
		...
		DeskKit.clock_chip(b, 800.0, 12.0, "%s's offer expires in %d wk" % [
			String(cand.get("name", "someone")).split(" ")[0],
			maxi(int(off.get("expires_wk", 0)) - state.week, 0)])
	var t2: Label = b.label("interviews cost founder time",
		Vector2(740.0, 44.0), 18, Color(DeskKit.INK, 0.5), 380.0)
	...
	if cand2.is_empty():
		DeskKit.empty(b, Vector2(z3.content_x, z3.cursor + 8.0),
			"nobody is at the offer stage.",
			"interview a candidate — the composer opens on whoever you pick", true)
	else:
		_composer(b, state, z3, cand2)


## THE COMPOSER: steppers left, the WILL-SHE-SAY-YES ticket right, SEND armed.
static func _composer(b, state: GameState, z3: Dictionary, cand: Dictionary) -> void:
	...
	DeskKit.ticket(b, float(z3.content_x) + 510.0, float(z3.cursor) - 2.0, 380.0, {
		...
		"lines": [
			{"label": "her ask", "value": "$%s %s" % [SimOwnership.money(ask),
				"cash-leaning" if mercenary else "mission-leaning"]},
			{"label": "this mix reads", "value": reads},
		],
		"total_label": "acceptance odds", "total_value": "≈%d%%" % int(round(odds)),
		"total_col": DeskKit.SAGE if odds >= 60.0 else DeskKit.PEN,
		"foot": "+$30 cash %+dpts · +0.2%% opt %+dpts" % [int(round(d_cash)), int(round(d_opt))]})
	var arm_y := float(fr.bottom) + 10.0
	if SimLabor.seats_left(state) <= 0:
		b.label("the house is full — no desk to offer", Vector2(float(z3.content_x), arm_y),
			DeskKit.DETAIL, Color(DeskKit.INK, 0.45), 360.0)
	else:
		DeskKit.arm(b, "send_offer", "SEND THE OFFER", "press again — the offer goes out",
			Vector2(float(z3.content_x), arm_y), func() -> void:
				SimOwnership.op_send_offer(b.state, cid, cash, opt)
				b.desk.erase("cash")
				b.desk.erase("opt"), 360.0, DeskKit.DETAIL)
