...
| N11 | **A headless run measures nothing.** | everywhere | `-nographics` renders no frame, so every render number is a frozen zero — the same warning the Godot probe prints for a non-windowed run. | `SystemInfo.graphicsDeviceType == Null` raises a **BLIND RUN** banner at the top of the file and a `BLIND` note on every row. |
...
| `RUNWAY_PERF_BLAME=1` (which node is repainting, by name and class) | Not ported | Godot can connect to every `CanvasItem.draw`; UGUI has no per-element rebuild callback, and the queues can only be COUNTED, not attributed, from outside. Naming the storm needs the Profiler window, not this file. |
