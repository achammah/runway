...
| DB-B4 | **`RunwayPaths.ArtExists` asks whether a PNG is ON DISK, not whether the picture is available — so the PNGs CANNOT leave `StreamingAssets` yet.** | 18 runtime call sites; the ones this pass newly exposes are `TitleScreen` (the film), `KeysScreen` (the mascot loop), `FounderDraftScreen` (the stage backdrop) and `DraftLoop.Hydrate` (frames 2 onward) | This is the reason this pass is **+61.0 MB of app size, not -22.6 MB**. Each of those asks "is the PNG there?" and takes a drawn fallback, or stops hydrating, when it is not. Delete the now-baked PNGs from `StreamingAssets` and the title plays its static still, the keys screen loses its mascot, the select stage loses its backdrop and every founder loop freezes on frame 1 — with the imported textures sitting right there. | Nothing was deleted. Both copies ship this pass: 61.0 MB of block-compressed data enters `resources.assets`, and 91.9 MB of PNG stays in `StreamingAssets`. The unlock is the pattern `DiceRoll.SheetOnHand` already uses (DF-B7): ask `ArtCache.HasBaked(rel) \|\| RunwayPaths.ArtExists(rel)`. Four call sites for the newly-exposed set, and then dropping those PNGs turns this pass into **-30.9 MB net**. Note this exposure is not new in kind — the six title sheets and 20 dice have had it since they were staged, which is why `DiceRoll` already carries the fix. |
...
| DB-B11 | **`StreamSequence`'s baked branch runs for the first time, and it loads all 48 frames at once.** | `SheetLoop.StreamSequence`, `TitleScreen.OnBuild` | That branch has been in the file since the sheets landed, but `BakedFrame` always returned null until now, so it had never executed. It calls `Resources.Load` 48 times in one go: 36 MB of DXT1 read and uploaded inside a single `OnBuild`. | It is what the branch is for — the loop must not play over a growing frame count, which was the title's visible jank. The thing it replaces is far worse: 48 `UnityWebRequest` decodes producing 288 MB of RGBA32, three at a time, with the loop holding on the newest frame throughout. **Worth a number**: the energy probe's `01 title` first-visit peak is the column to watch, and if 36 MB of synchronous read is too much for one frame, the branch can be paced the same way the stream was, without touching anything else. |
...
- **The art lane owns the 150-file source-dimension pass**, listed above. It is worth
  ...
  lever.
- **Four `ArtExists` guards own the app-size half** (DB-B4). Until `TitleScreen`,
  `KeysScreen`, `FounderDraftScreen` and `DraftLoop.Hydrate` ask whether a picture is
...
- **`.gitignore` grew one line**, `unity/Assets/Resources/Art/`, beside the
  ...
  `Assets/Art`. **`unity/Assets/Resources/Art.meta` still needs committing**: the
  folder's own meta sits outside the ignored directory and carries the folder GUID,
  exactly as `Resources/Sheets.meta` is committed today.
